...
function WSSecurity(username, password, passwordType, options) {
  this._username = username;
  this._password = password;
  this._passwordType = passwordType || 'PasswordText';
  this._addNonce = true;
  this._addCreated = true;
  if (options && options.hasOwnPropertyName("addNonce")) {
    this._addNonce = options.addNonce;
  }
  if (options && options.hasOwnPropertyName("addCreated")) {
    this.addCreated = options.addCreated;
  }
}
...
WSSecurity.prototype.toXML = function() {
  ...
  return  "<wsse:Security xmlns:wsse=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd\" xmlns:wsu=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\">" +
    ...
    (this._addNonce ? "<wsse:Nonce EncodingType=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary\">" + nonce + "</wsse:Nonce>" : "" ) +
    (this._addCreated ? "<wsu:Created>" + created + "</wsu:Created>" : "" ) +
    "</wsse:UsernameToken>" +
    "</wsse:Security>";
};
